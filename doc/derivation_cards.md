...
A problem on this page has been attempted eight to thirty-eight times and is still marked
`again`. The pattern is not the problem — the pattern is on a cheatsheet the reader already
...
- **Invariant** — indices on the stack have temperatures strictly decreasing bottom to top, and none has yet seen a warmer day.
...
- **The line that sets the complexity** — `if ra != rb: parent[ra] = rb; count -= 1` — O(E α(n)). The DFS version is O(V + E) with an adjacency list and a visited set; both are fine, name the one you write.
...
- **Contract** — neighbours by trying 26 letters at each position and checking the set: `O(L · 26)` per word versus `O(N · L)` for comparing against every other word — that is the choice that makes it pass.
- **The line that sets the complexity** — the neighbour loop `for i in range(L): for c in ascii_lowercase:` — O(N · L · 26) total, since each word is enqueued once.
