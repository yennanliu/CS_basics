...
Cycle detection on a prerequisite graph. Worked in full — DFS with a `visiting` path set,
and Kahn's in-degree queue — in
[topology_sorting_examples.md](./topology_sorting_examples.md#2-course-schedule--lc-207-),
which owns the ordering family. [2-8) Find Eventual Safe States](#2-8-find-eventual-safe-states--lc-802)
below is the same three-colour DFS asked the other way round.
