...
> **Scope** — The worked-solution archive behind [matrix.md](./matrix.md): seventeen problems grouped by the geometry or technique each one turns on — traversal order, in-place transformation, staircase search, grid search, 2D DP, and row-pair compression.
> **See also**: [matrix.md](./matrix.md) — the parent sheet: the traversal templates, the index↔coordinate arithmetic and the pattern-selection strategy these solutions apply; [dfs.md](./dfs.md) and [bfs.md](./bfs.md) — grid search in its own right; [prefix_sum.md](./prefix_sum.md) — the 2D prefix sum and row-pair compression theory; [dp.md](./dp.md) — grid DP; [monotonic_stack.md](./monotonic_stack.md) — the histogram step inside LC 85.
...
This is the long tail of [matrix.md](./matrix.md). The parent keeps the templates, the essential
matrix properties and the pattern-selection strategy; this file keeps the problems that *apply*
them.
...
### 9) Search a 2D Matrix II — LC 240 — staircase elimination ⭐⭐⭐⭐
