...
> current DFS path, so a repeat inside it means a cycle, and forgetting the removal turns the
> path set into a global seen-set that reports a cycle for any re-visited node.

The solution itself is in
[topology_sorting_examples.md](./topology_sorting_examples.md#2-course-schedule--lc-207-),
with the Kahn / in-degree treatment beside it.
