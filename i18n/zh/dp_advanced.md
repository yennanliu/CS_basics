...
**這條規則在哪裡就不通用了**：硬幣是*相加*的，所以任何 multiset 都可以重排成硬幣迴圈的順序 —— 這正是硬幣放外層之所以合法的原因。單字則是接在字串早就固定好的位置上，所以 LC 139 Word Break 根本沒有合法的 item-在外寫法。判準與 LC 139 的五種寫法：[dp_loop_order.md](./dp_loop_order.md)。
