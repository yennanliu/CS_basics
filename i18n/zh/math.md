...
<!-- ed8b8abb50b0 -->
#### 1-1-2') 取得 `x` 的因數 — 因數(divisor)與質因數(prime factor) ⭐⭐⭐⭐

「因數」這個詞底下其實藏了兩個不同的問題。兩者都靠**同一個 `while d * d <= x` 迴圈**解決，所以把這個迴圈學起來一次就夠：

| 問題 | `x = 12` 時的答案 | 答案的規模 | 成本 |
|----------|---------------------|----------------|------|
| **所有因數** — 每個滿足 `x % d == 0` 的 `d` | `1, 2, 3, 4, 6, 12` | 很小（`x ≤ 10⁵` 時 ≤ 128 個） | O(√x) |
| **質因數** — 相乘會等於 `x` 的那些質數 | `{2, 3}`（即 `2² × 3`） | `x ≤ 10⁹` 時相異的約 ≤ 15 個 | O(√x) |

**為什麼掃到 `√x` 就夠了（兩者共同的關鍵想法）：**

<!--CODE-->

<!-- 58480a5c64ed -->
##### a) `x` 的所有因數 — 掃到 `√x`，每組配對取兩邊
...
<!-- 69807d744de1 -->
##### b) `x` 的相異質因數 — 找到一個就把它整個「除乾淨」

這一段是主力。讓它同時正確**又**快速的技巧是：只要 `d` 整除 `x`，就在往下走之前把 `x` 裡的 `d` **全部**除掉。這麼做之後，`x` 再也不可能被 `d` 整除，所以之後任何能整除的 `d` 自動就是**質數** — 整段程式完全不需要做質數判斷。
...
**兩個最常被寫錯的地方：**
1. **漏了最後的 `if x > 1`。** 以 `x = 14` 為例，迴圈只跑到 `d = 3`（`3*3 > 7`），剩下的 `7` — 一個大於 `√14` 的質數 — 就會被漏掉。這種漏網的質數最多只會有一個，因為兩個 `> √x` 的質數相乘會超過 `x`。
2. **先去判斷 `d` 是不是質數。** 沒必要：走到那裡時 `4` 根本不可能整除 `x`，因為所有的 `2` 早就被除掉了。

**也想要指數嗎？** 在除的時候順便計次，就會得到 `360 -> {2:3, 3:2, 5:1}`，這正是計算因數個數（`∏(eᵢ+1)`）與尤拉函數(totient)公式需要的形式。見 [`combinatorics_math_patterns.md`](./combinatorics_math_patterns.md) *Prime Factorization*。

<!-- 5a718fc61224 -->
##### c) 一次分解**很多**數字 — 最小質因數(SPF)篩法

試除法對**每個**數字都要 O(√x)。如果輸入是一個最多 `10⁵` 個、每個值最大到 `10⁵` 的陣列，就先篩出每個數的最小質因數，之後每次分解都只要 O(log x)。
...
> 一個子陣列的質因數集合，是其所有元素的相異質因數的**聯集**。求聯集大小最多為 `k` 的最長子陣列。

**為什麼滑動視窗可行**：視窗變大只會往聯集裡**加**質數，不會拿掉任何一個 — 聯集大小對視窗是單調的，所以經典的 `右邊擴張／不合法就從左邊收縮` 模板就適用。維護一個 `質數 -> 視窗中有幾個元素含有它` 的計數表；聯集大小就是**還活著的鍵**的個數。

<!--CODE-->

<!--CODE-->

> 在 `nums[i] <= 10⁵`、`n <= 10⁵` 之下，試除法約是 3×10⁷ 次運算 — 對 Java 很輕鬆，對 Python 則在邊緣。把 `get_prime_factors` 換成上面的 **SPF 篩法**，分解那一步就會降到 O(n log A)。

**「取得因數」會在哪些題目出現：**

| 題目 | 因數被拿來做什麼 |
|---------|-------------------------------|
| LC 4032 - Longest Subarray With at Most K Distinct Prime Factors | 對質數集合的聯集做滑動視窗 |
| LC 952 - Largest Component Size by Common Factor | 併查集：把每個 `x` 和它的每個質因數 union 起來 |
| LC 1998 - GCD Sort of an Array | 同樣的想法 — 把共用質數的索引連起來 |
| LC 2521 - Distinct Prime Factors of Product of Array | 對整個陣列取 `get_prime_factors(x)` 的聯集 |
| LC 1390 - Four Divisors | 用 `get_divisors`，留下剛好有 4 個的 |
| LC 1492 - The k-th Factor of n | 用 `get_divisors` 由小到大排，取索引 `k-1` |
| LC 204 - Count Primes | 改用篩法 — 見 [1-1-2](#1-1-2-count-prime-number) |
...
| LC 1492 / 1390 / 2521 / 952 | 「取得 `x` 的因數」— `√x` 試除法，或對整個陣列用 SPF 篩法 | [1-1-2'](#1-1-2-get-factors-of-x--divisors--prime-factors-) |
