...
| 我想要… | 前往 |
|---|---|
| 知道某個內建操作要花多少成本 — `len`、`max`、`in`、`+`、`split`、切片、`dict`/`set` 操作 | [python_complexity.md](./python_complexity.md) |
...
基本操作就是一般 `dict` 的操作——它*本身就是* `dict` 的子類別，所以索引、`get`、`in`、`len`
與 `del` 的行為都和你預期的完全一樣。只有跟順序有關的呼叫是額外多出來的：
