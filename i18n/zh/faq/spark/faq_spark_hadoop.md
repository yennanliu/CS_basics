...
- ***Spark*** 同樣是存取大規模資料集的大數據框架，並負責 ETL、串流、機器學習等處理……
	...
	- `可以把資料集跨階段留在記憶體裡`。RDD 是*延遲求值的邏輯*資料集，不是已經被快取的東西 —— 只有在你 `cache()`/`persist()` 之後它才留在記憶體，而放不下的分區會怎樣取決於 `StorageLevel`：`cache()` 與預設的 `persist()` 是 `MEMORY_ONLY`，下次用到時會**重算**它；`MEMORY_AND_DISK` 才是溢寫到磁碟。Spark 通常比較快，是因為它避開了每個階段的磁碟來回、又能把窄轉換融合起來，不是因為所有東西永遠都在記憶體裡
