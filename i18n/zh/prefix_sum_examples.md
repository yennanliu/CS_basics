...
- **核心想法**：八題裡有四題其實是同一招 — 用雜湊表記錄「前綴值 → 出現過幾次」 — 只是套在輸入的不同轉換上
...
**模式：**把後綴極值真的存出來 + 前綴極值邊走邊滾
- 一趟從右到左建好 `suffixMin` —— O(n) 時間、O(n) 空間
- 一趟從左到右同時回答兩個判斷並更新 `prefixMax` —— O(n) 時間、額外 O(1)
- beauty 為 2 的那個判斷，就是 LC 768 的切點判斷（`prefixMax < suffixMin`），只是問的是*單一個元素*而不是切點
...
| 陷阱 | 會出什麼錯 |
|---|---|
| 極值的索引差一 | 兩邊的視窗都是*嚴格*的。用 `suffixMin[i] = min(suffixMin[i+1], nums[i])` 建出來的會包含 `nums[i]` 自己，所以之後必須讀成 `suffixMin[i+1]`。改成（上面那樣）從 `nums[i+1]` 開始建，才能直接在 `i` 讀。兩種寫法混用，每個 beauty-2 的判斷就變成拿 `nums[i]` 跟自己比，答案直接崩掉。 |
| 用 `if` 而不是 `elif` | beauty 為 2 的元素同時也滿足鄰居判斷，所以多寫一個 `if` 會把它算成 3。 |

> **為什麼前綴那一側不需要陣列。**`prefix_max` 只會在迴圈當下那個索引被讀取，而且是在每一輪
> 結尾才更新 —— 所以手上那個值永遠就是 `max(nums[0 .. i-1])`。後綴那一側則是在「本來要算它的
> 那一趟」*之前*就被讀取，這正是兩個方向裡必須有一個要存下來的原因。

**類似題：**

| 題目 | LC # | 關鍵差別 |
|---------|------|----------------|
| Max Chunks To Make Sorted II | 768 | 同樣的 `prefixMax < suffixMin` 判斷，但問的是切點而不是元素 —— 模板 8 |
