...
| 動態中位數 | 兩個堆積 | 每次加入之後 | 見 `heap.md`（LC 295, 480） |
...
> **交叉參考：** 動態中位數（兩個堆積）放在 `heap.md`，延遲刪除的堆積放在 `heap_advanced.md`；這裡的串流視角只強調一件事 ——「記憶體有界為 k，n 無上限」。
...
| LC 480 | Hard | 兩個堆積 + 延遲刪除 | LC 295 的視窗版 —— 見 `heap.md` |
