...
<!-- 16911332ab3d -->
### 模板 2a：讀懂二維 DP 表格 — 形狀、相依、填表順序 ⭐⭐⭐⭐⭐

模板 2 是最輕鬆的二維情況：表格本身*就是*輸入的格子，`dp[i][j]` 早就被題目放好了。另外兩個二維
家族 — 區間 DP（模板 3、模板 10）與雙序列 DP（模板 8、模板 9）— 憑空生出一張題目沒有給你的表格，
...
> 合法，當且僅當每支箭頭都落在**已經填好**的格子上。就只有這樣而已。

| 轉移式讀到 | 是哪個鄰居 | 迴圈就必須 |
|---|---|---|
| `dp[i-1][j]` | **上**一列 | `i` 遞增（`1 → m`） |
| `dp[i+1][j]` | **下**一列 | `i` **遞減**（`n-1 → 0`） |
| `dp[i][j-1]` | **左**一行 | `j` 遞增（`1 → n`） |
| `dp[i][j+1]` | **右**一行 | `j` **遞減** |
| `dp[i+1][j-1]` | **內縮**後的區間（兩端各往內一格） | `i` 遞減**且** `j` 遞增 |
| `dp[i][k]`、`dp[k][j]`（`i < k < j`） | **更短**的區間 | 依區間**長度**遞增 |
...
<!-- 6036d3d0025c -->
#### 形狀 A — 單一序列，`i`/`j` 是區間的兩端

**狀態**：`dp[i][j]` = 子區間 `s[i..j]` 的答案。

<!--CODE-->

**相依關係** — 一個「兩端往內縮」的轉移會讀到的三個格子：

<!--CODE-->

<!-- 7fdcccb32e8f -->
##### 順序 A — `i` 倒著走、`j` 正著走（就寫這個）
...
<!-- fc48fcc0102c -->
##### 兩種都合法 — 以下是格子實際被填的順序（`n = 5`）

<!--CODE-->

**怎麼選**：轉移移動的是兩**端**（`dp[i+1][j-1]`、`dp[i+1][j]`、`dp[i][j-1]`）就用順序 A — 回文、
拿字串跟自己的反轉取 LCS。轉移需要區間*內部*的**切點 `k`**（`dp[i][k] + dp[k][j]`）就用順序 B —
戳氣球、合併石頭、矩陣連乘 — 因為在那裡「已經填好」指的是*嚴格更短*，而只有依長度驅動的迴圈保證
得了這件事。

<!-- c618590b00cf -->
##### 實際填好的表 — LC 516，`s = "bbbab"`
...
正向那一趟存下來的是*數值*。**決策**是事後在每個格子重問一次「這個值是哪個鄰居給的」還原出來的 —
不需要父指標，把轉移重算一次就好：
...
| **LC 5** Longest Palindromic Substring | `j - i <= 2` 或 `dp[i+1][j-1]` 時為 `True` | `False` |
| **LC 647** Palindromic Substrings | 判斷同 LC 5，數有幾個 `True` | `False` |
| **LC 1312** Min Insertions to Make Palindrome | `dp[i+1][j-1]` | `1 + min(dp[i+1][j], dp[i][j-1])` |

有兩題完全沒有字元比對，而它們正好就是替你決定填表順序的那種：

- **LC 312**（Burst Balloons）— 有**切點**，所以只有順序 B 能用：
  `dp[i][j] = max over k in (i, j) of dp[i][k] + dp[k][j] + b[i]*b[k]*b[j]`。模板 3 有完整解法。
- **LC 877 / LC 486**（Stone Game / Predict the Winner）— 同一個三角形，但 `dp[i][j]` 裝的是
  **分數差**：`max(nums[i] - dp[i+1][j], nums[j] - dp[i][j-1])`。動的是兩端，所以用順序 A。
  minimax 的推理在 [dp_advanced.md](./dp_advanced.md)。
...
**狀態**：`dp[i][j]` = **`s1` 的前 `i` 個字元**與 **`s2` 的前 `j` 個字元**的答案。所以這個格子在決定的
字元是 `s1[i-1]` 與 `s2[j-1]` — 大家都會踩到的那個差一，只是第 `0` 列／第 `0` 行代表*空前綴*的後果。

<!--CODE-->

**相依關係** — 三支箭頭，全都從上方／左方／左上方過來：

<!--CODE-->

**每一步各代表什麼** — 遞推式會隨題目改變，*幾何*永遠不變：

| 走法 | 讀到的格子 | 意義 | LC 72（編輯距離） | LC 1143（LCS） |
|---|---|---|---|---|
| ↘ 對角 | `dp[i-1][j-1]` | 兩邊**各**吃掉一個字元 | 相同免費，否則**取代**（`+1`） | 相同就 `+1` |
| ↓ 垂直 | `dp[i-1][j]` | 只吃 `s1` 一個字元 | **刪除** `s1[i-1]`（`+1`） | 跳過 `s1[i-1]` |
| → 水平 | `dp[i][j-1]` | 只吃 `s2` 一個字元 | **插入** `s2[j-1]`（`+1`） | 跳過 `s2[j-1]` |

<!--CODE-->

> **基底那一列才是 bug 的住處。** 編輯距離是 `dp[i][0] = i`，LCS 是 `0`，LC 115 是 `1` — 空字串是
> 任何字串的子序列，而且恰好一次。要用推的，不要用抄的：把 `i = 0` 代進你自己的狀態句子，再用白話
> 把答案唸出來。

<!-- 57ae8da5e982 -->
##### 實際填好的表 — LC 72，`"horse"` → `"ros"`，含它選的那條路

<!--CODE-->

用幾何的方式讀這條路：**↘ 代表某個字元活了下來，↓ 代表花掉 `s1` 的一個字元，→ 代表花掉 `s2` 的
一個字元。** 同樣這一趟反向走法套在 LC 1143 的表上，拼出來的就不是操作清單，而是真正的最長公共
子序列 — 同一張表、同一趟走法，只是沿路記下來的東西不同。

<!-- 15797bb11ca1 -->
##### 空間壓縮 — 以及什麼時候沒得壓

`dp[i][j]` 只會讀到第 `i` 列與第 `i-1` 列，所以形狀 B 可以塌成**兩列**，再塌成一列加一個純量
（用來接住那個即將被覆蓋掉的對角值）：

<!--CODE-->

**形狀 A 壓不了。** `dp[i][j]` 讀的是第 `i+1` 列的*兩個不同行*（`j-1` 和 `j`），而斜線掃描又會一直
往回摸到更早的每一條斜線 — 根本沒有「最近兩列」這個可以留著的視窗。區間 DP 就編列 O(n²)，然後往
下走。

還有，只要你需要反向回溯就完全不要壓：滾動列丟掉的，剛好就是反向那一趟要讀的格子。只要答案是那條
*路徑*而不是那個數字，就把整張表留著（或把每個格子選了哪一步記下來）。

<!-- 053ce431317e -->
##### 形狀 B 的題目

| 題目 | `s1[i-1] == s2[j-1]` 時 | 否則 |
|---|---|---|
...
還有兩題住在同一張網格上，卻沒有 `s1` 跟 `s2` 互比字元這件事：

- **LC 97**（Interleaving String）— 布林值，而且是拿去跟*第三個*字串比：
  `dp[i][j] = (dp[i-1][j] and s1[i-1] == s3[i+j-1]) or (dp[i][j-1] and s2[j-1] == s3[i+j-1])`。
  完全沒有對角線 — 只有 ↓ 和 →，這正是「每個字元恰好來自兩個來源之一」的形狀。
- **LC 10**（Regular Expression Matching）— `j` 軸是*樣式*。一般字元或 `.` 就是普通的 ↘；`'*'` 會
  裂成「零份」`dp[i][j-2]` 與「再多一份」`dp[i-1][j]`，兩者讀的仍然只有左邊和上面，所以照列掃還是
  成立。
...
| | **形狀 A — 單一序列** | **形狀 B — 雙序列** | **模板 2 — 真的格子** |
|---|---|---|---|
| `dp[i][j]` 的意思 | 區間 `s[i..j]` | `s1` 前 `i` 個 vs `s2` 前 `j` 個 | 座標 `(i, j)` 這一格 |
| 表格大小 | `n × n`，上三角 | `(m+1) × (n+1)` | `m × n` |
| 基底情況 | 對角線 `dp[i][i]` | 第 0 列與第 0 行 | 第一列與第一行 |
| 答案在哪 | 右上角 `dp[0][n-1]` | 右下角 `dp[m][n]` | 右下角 `dp[m-1][n-1]` |
| 讀到 | `dp[i+1][j-1]`、`dp[i+1][j]`、`dp[i][j-1]` | `dp[i-1][j-1]`、`dp[i-1][j]`、`dp[i][j-1]` | `dp[i-1][j]`、`dp[i][j-1]` |
| 迴圈順序 | `i: n-1 → 0`、`j: i+1 → n-1`（或依長度） | `i: 1 → m`、`j: 1 → n` | `i: 0 → m-1`、`j: 0 → n-1` |
| 時間／空間 | O(n²) / O(n²) — 有切點時 O(n³) | O(m·n) / 滾動後 O(min(m,n)) | O(m·n) / 滾動後 O(n) |
| 代表題 | LC 516、5、647、1312、312 | LC 72、1143、115、97 | LC 62、64、221 |

<!-- fd37134429ff -->
#### 真正會毀掉這張表的四個錯

1. **填錯方向。** 形狀 A 的 `i` 一定要遞減；寫成 `for i in range(n)` 會在 `dp[i+1][...]` 還是 `0`
   的時候就去讀它，答案於是悄悄偏小 — 不會爆、不會有索引錯誤。方向請從上面那張箭頭表推出來，不要
   用背的。
2. **踩進死掉的三角形。** 形狀 A 裡當 `j == i + 1` 時，`dp[i+1][j-1]` 就是 `dp[i+1][i]`，在對角線
   下方。它天生是 `0` / `False` — 這對 LC 516 剛好*正確*（`0 + 2 = 2`，兩個字元的回文），對 LC 5
   卻是*錯的*，那正是後者需要明確寫出 `j - i <= 2` 這個護欄的原因。
3. **把別題的基底列抄過來。** 同一個形狀裡，`0`、`i`、`1` 對不同題目都各自是正確的基底列 — 見上面
   那段提醒。
4. **還沒確定只要數字就先壓空間。** 滾動列只有在答案是一個值的時候才是免費的；追問一變成「那是哪些
   操作？」，你就得把整張表要回來。

<!-- 54d92372c347 -->
### 模板 3：區間 DP — LC 312

> 這張表是**形狀 A** — `n × n`、上三角、答案在右上角。它的骨架、兩種合法的填表順序，以及為什麼有
> 切點就只能用依長度驅動的那一種，都在
> [模板 2a](#template-2a-reading-a-2-d-dp-table--shape-dependency-fill-order-)。
...
這張表是**形狀 B** — 它的形狀、為什麼照列掃是唯一合法的順序，以及把操作清單還原出來的反向走法，
都在 [模板 2a](#template-2a-reading-a-2-d-dp-table--shape-dependency-fill-order-)。
...
> 這張表是**形狀 A** — 下面那個 `i` 倒著走、`j` 正著走的順序就是
> [模板 2a](#template-2a-reading-a-2-d-dp-table--shape-dependency-fill-order-) 裡的順序 A；而
> `j - i <= 2` 這個護欄之所以存在，是因為 `dp[i+1][j-1]` 會掉進死掉的三角形裡。
