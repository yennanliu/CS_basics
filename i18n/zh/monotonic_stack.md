...
改用 `(值, 步數)` 的配對堆疊完全等價，還可以省掉 `dp` 陣列 — 推入 `(num, cur)` 而不是索引，彈出條件改成 `stack[-1][0] <= num`。當索引沒有別的用途時就用這種寫法。
...
| Car Fleet II | 1776 | 由右往左的堆疊，每台車的碰撞時間由被彈掉的車推導而來 — 一樣的串接，只是 dp 是實數 |
| Car Fleet | 853 | 同樣把模擬收斂成堆疊，但答案是*存活的車隊數*，不是誰在第幾輪死掉 |
