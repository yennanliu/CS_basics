...
| 類別 | 它回答的問題 | 模板 | 例題 |
|----------|--------------------|----------|----------|
| **LCA（有 root）** | `p` 和 `q` 在哪裡交會？ | 後序 DFS，回傳第一個同時看到兩者的節點 | LC 236, 1644, 1676, 865, 1123 |
| **LCA（沒有 root）** | 同樣的問題，但手上只有 `p`、`q` 和 `parent` 指標 | **往上**走兩條父鏈 — 其實就是換皮的 LC 160 | LC 1650 |
| **LCA（有序樹）** | 同樣的問題，但樹是 BST | 只要兩個目標都在同一側就**往下**走 | LC 235 |
...
這五題都在講「最近共同祖先」。但上面那個後序模板只適用第一列；其餘各改掉一個前提，
而那個前提就決定了解法的形狀。

| LC | 跟 236 差在哪 | 對應的形狀 | 複雜度 |
|----|--------------------|-----------------|------------|
| **236** | — （基準題：給 root，`p`、`q` 保證存在） | 後序，回傳「找到或 null」 | O(N) / O(H) |
| **235** | 樹是 **BST** | 不用兩邊都遞迴 — 只要 `p`、`q` 在同一側就**往下**走 | O(H) / O(1) |
| **1644** | `p`、`q` **可能不存在**於樹中 | 一樣的後序，但還要數看到幾個目標；只有 `count == 2` 才算答案 | O(N) / O(H) |
| **1650** | **沒有 root** — 每個節點有 `parent` 指標 | 往**上**走而不是往下 → 這就是 LC 160（兩條鏈結串列） | O(H) / O(1) |
| **1676** | 目標有 **N 個**而不是 2 個 | 一樣的後序，只是比對一個目標 `set` | O(N) / O(H) |

> **挑模板只要問一個問題**：*我能不能從某個 root 走到 `p` 和 `q`？*
> 能 → 往**下**遞迴（236 / 1644 / 1676）。如果我是*站在* `p`、`q` 上、手上只有
> `parent` 指標 → 往**上**走（1650）；這時根本沒有一棵樹可以搜。

<!-- 6562927d31b9 -->
#### LCA 變形 — 有父指標、沒有 root（LC 1650） ⭐⭐⭐⭐

<!-- 90292498dee7 -->
##### **1. 核心想法**

**關鍵洞察**：`lowestCommonAncestor(p, q)` — 注意 `root` 不見了。你沒有把手，就搜不了這棵樹。
但 `p.parent.parent…` 是一條**以 root 結尾的鏈結串列**，所以兩個節點給你兩條串列，
而它們合流的地方剛好就是 LCA。**這就是 LC 160, Intersection of Two Linked Lists**，
只是穿了一件樹的外衣。

<!--CODE-->

所以能用的兩種形狀，就是 LC 160 的那兩種：**把一條路徑放進 set**，或是**雙指標**。

<!-- 680fd76dcc4c -->
##### **2. 模板 A — 祖先 set（面試先講這個）**

<!--CODE-->

> **為什麼第一個命中的就是最低的**：從 `q` 往上走，會按高度遞增的順序拜訪它的祖先，
> 所以最早遇到的共同節點就是最深的共同節點。這也是為什麼「插入有序的 dict」那個變體
> 同樣成立。
> **要 hash 節點，不是 `node.val`** — `val` 只是剛好唯一（題目條件這樣寫）；
> 節點物件本身則永遠唯一。

<!-- 8158baaf84ff -->
##### **3. 模板 B — 雙指標「換尾巴」（O(1) 空間） ⭐⭐⭐⭐⭐**

兩條父鏈的**長度不一樣**，所以兩邊同步往上走會錯過交會點。修法就是 LC 160 那招：
指標一走出頂端就把它重新放到*另一個*起點。這樣每個指標都剛好走
`len(path(p)) + len(path(q))` 步，於是兩者會在**同一輪**抵達共同尾巴的每一個節點。
...
**常見坑：**
- ❌ `a = a.parent or q` ／ `a = (a.parent != null) ? a.parent : q` — 這會**跳掉 `null` 那一步**。
  把 `null` 也走掉，才能讓兩邊的總步數相等；跳掉就永遠差一步，兩個指標會一直錯身而過。
- ❌ 用 `a.val != b.val` 而不是 `a != b` — 這題剛好可行，但你要的是節點的身分，
  值一旦重複就爆了。
- ❌ 以為 `p` 和 `q` 一定同根。若它們在**不同**的樹裡，兩個指標會在同一步一起落到 `null`，
  於是 `a == b == null`，迴圈結束回傳 `null` — 雙指標寫法白送你這個終止條件，
  單純同步往上的那種寫法沒有。
- ❌ 反射動作去拿後序模板。這裡根本沒有 `root` 參數可以傳給它。

<!-- 16234bed5194 -->
##### **4. 一次看完其他變形**

**LC 1644 — `p` / `q` 可能不在樹裡。** 236 的模板只要碰到 `p` 就回傳 `p`，
所以就算 `q` 不存在，它也會很開心地報一個「LCA」給你。修法：不要提早 return，
並且**數你真正看到幾個目標**。

<!--CODE-->

**LC 1676 — 目標有 N 個而不是 2 個。** 把那兩個 `==` 比對換成 `set` 成員檢查；其他一行都不用動。

<!--CODE-->

**LC 235 — 樹是 BST。** 有序性讓你不必看兩邊子樹：只要兩個目標都落在 `node.val` 的同一側，
LCA 就在那一側，直接走過去。
...
| LCA of a Binary Tree III | 1650 | — | 有父指標、**沒有 root** → 變成「兩條鏈結串列的交點」；見[上面的變形](#lca-variant--parent-pointers-no-root-lc-1650-) |
