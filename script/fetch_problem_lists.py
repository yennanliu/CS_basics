...
of PROBLEMS.md at build time and are not in this file.
...
def fetch_neetcode():
    ...
    for match in RECORD_RE.finditer(bundle):
        ...
        problems[problem_id] = {
            ...
            # Needed for problems this repo has no indexed row for — without a
...
def build():
    ...
    return {
        "_comment": (
            ...
            "script/fix_readme_tags.py (which writes them as index Note tags) — "
            ...
            "`google` and `must` lists are not here; they come from PROBLEMS.md."
        ),
        "sources": {
