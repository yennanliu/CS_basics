...
因為餘數 `r` 必定小於總組數 `k`（`r < k`），
我們**不可能給全部 `k` 個組別都再加 1**（那樣需要 `k` 個額度）。

為了滿足算術-幾何平均不等式（AM-GM Inequality）——**「數字彼此越接近，乘積越大」**，最平均的做法就是：

- 拿剩下來的 `r` 個額度，**給其中 `r` 個組別各加 1**。
- 這 `r` 個組別的值就會變成 **`q + 1`**。
- 剩下的 `k - r` 個組別沒有分到餘數，保持基準值 **`q`**。
