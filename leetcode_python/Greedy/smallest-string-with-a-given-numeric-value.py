...
characters' numeric values. For example, the numeric value of the string "abe" is equal to
1 + 2 + 5 = 8.
...
# V0
# IDEA : GREEDY FROM THE BACK (keep the prefix as small as possible)
#
#   lexicographically smallest -> every EARLY position must be as small as it can,
#   which means we should dump all the surplus value into the LAST positions.
#
#   start from "aaa...a" (value n), surplus d = k - n must be spread out.
#   walk from the right, giving each slot the maximum extra it can take (25 -> 'z')
#   until the remaining surplus fits in one slot.
#
#   NOTE : k >= n is guaranteed, so d >= 0 and the answer always exists.
#
# time = O(n), space = O(n)
