...
3.  為什麼我們不用去考慮`局部`的 `min(height) * width`（例如用高度 1 去涵蓋寬度 2）？


->


    因為「以該高度為準、向左右擴展到極限」的最大寬度矩形，
    已經自動把那些局部的小寬度矩形包辦了，而且面積一定更大或相等！
...
-> 
    ...
    假設你有一個區間，其中的最小高度是 1，寬度是 2（例如包含高度為 2 和 1 的柱子）。



    - 如果你算「高度 1 乘以寬度 2」，面積是 $1 \times 2 = 2$。

    - 但實際上，高度為 1 的這個柱子，它的左右邊界可以向外擴展（
      只要遇到比它高的柱子都可以吃下來）。
      當它擴展到極限時，寬度可能是 3（涵蓋了後面的 5），
      面積變成 $1 \times 3 = 3$。
    ...
    -> 
        ...
        結論：在高度相同的情況下，寬度越大，面積就越大。
             我們只需要讓每個高度去尋找它「所能延伸的最大左右邊界」，
             產生的最大面積一定會涵蓋並超越所有局部的子區間組合。
