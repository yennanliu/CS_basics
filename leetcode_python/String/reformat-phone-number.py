...
# V0
# IDEA : STRING SIMULATION DRIVEN BY len(digits) % 3
#
#   strip separators, then cut blocks of 3 greedily. only the TAIL is special and
#   it is fully decided by n % 3 :
#     n % 3 == 0 -> all blocks are 3
#     n % 3 == 2 -> full 3-blocks, then one 2-block
#     n % 3 == 1 -> a lone digit would be left, so borrow from the previous block:
#                   shrink the last 3-block to 2 and make the leftovers a 2-block
#
#   NOTE : the "never produce a block of length 1" rule is exactly the n % 3 == 1 fix.
#
# time = O(n), space = O(n)
class Solution(object):
    def reformatNumber(self, number):
        ...
        n = len(digits)
        blocks = [digits[i:i + 3] for i in range(0, n - n % 3, 3)]
        r = n % 3
        if r == 1:
            # last full block gives one digit back -> two blocks of 2
            blocks[-1], tail = blocks[-1][:2], blocks[-1][2] + digits[-1]
            blocks.append(tail)
        elif r == 2:
