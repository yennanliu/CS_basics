...
# V0
# IDEA : UNION FIND (26 letters), root of a group is always its SMALLEST letter
#
#  key trick : when merging, point the bigger root at the smaller root,
#              so find(c) always returns the lexicographically smallest
#              character equivalent to c
# time = O((n + m) * a(26)), n = len(s1), m = len(baseStr)
# space = O(26)
class Solution(object):
    def smallestEquivalentString(self, s1, s2, baseStr):
        ...
        for a, b in zip(s1, s2):
            ...
            if ra == rb:
                continue
            # NOTE !!! always keep the SMALLER letter as the root
            if ra < rb:
                parent[rb] = ra
            else:
                parent[ra] = rb
