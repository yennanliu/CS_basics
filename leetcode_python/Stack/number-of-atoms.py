...
the second name (in sorted order), followed by its count (if that count is more than 1),
and so on.
...
#   At the end stack[0] holds the totals; format them in sorted name order.
#
# time = O(n^2) worst case (n = len(formula)) — merging a popped group is O(distinct atoms)
# space = O(n)
