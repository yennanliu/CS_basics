...
時間複雜度： O(N) — 每個元素最多只會被壓入（Push）和彈出（Pop）棧一次。

空間複雜度： O(N) — 最壞情況下（嚴格遞減陣列），棧最多儲存 $N$ 個元素。
...
| 步驟 | `num` | 棧狀態（彈出前） | 二分/彈出過程與 `cur_steps` | 最終 `cur_steps` | 壓棧後 `stack` 內容 | 全局 `max_steps` |
| --- | --- | --- | --- | --- | --- | --- |
| **1** | **5** | `[]` | 棧為空 $\rightarrow$ 永遠不被消除 | `0` | `[(5, 0)]` | **0** |
| **2** | **3** | `[(5, 0)]` | 棧頂 `5 > 3` $\rightarrow$ 被 5 消除 (`0 + 1`) | `1` | `[(5, 0), (3, 1)]` | **1** |
| **3** | **4** | `[(5, 0), (3, 1)]` | 彈出 `(3, 1)`，`cur_steps = max(0, 1) = 1`；棧頂剩 `5 > 4` | `2` | `[(5, 0), (4, 2)]` | **2** |
| **4** | **4** | `[(5, 0), (4, 2)]` | 彈出 `(4, 2)`，`cur_steps = max(0, 2) = 2`；棧頂剩 `5 > 4` | `3` | `[(5, 0), (4, 3)]` | **3** |
| **5** | **7** | `[(5, 0), (4, 3)]` | 彈出 `(4, 3)` 與 `(5, 0)`，`cur_steps = 3`；棧空 | `0` | `[(7, 0)]` | **3** |

**最終結果**：`max_steps = 3`（對應過程：第一輪刪除 3；第二輪刪除第一個 4；第三輪刪除第二個 4）。
