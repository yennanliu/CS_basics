...
# V0
# IDEA : CUSTOM SORT KEY + STABLE SORT
"""
 Build a tuple key per log:

    letter-log -> (0, content, identifier)   # letters first, then by content, then id
    digit-log  -> (1, "", "")                # all digit-logs compare EQUAL

 Python's sort is stable, so all digit-logs (identical keys) keep their original
...
class Solution(object):
    def reorderLogFiles(self, logs):
        def key(log):
            ...
            # only need to test the first char of the content:
