...
#   "best beauty at price <= q" = the prefix-max at the last index whose
#   price is <= q — a bisect_right on the price column.
#
...
class Solution(object):
    def maximumBeauty(self, items, queries):
        ...
        res = []
        for q in queries:
            i = bisect.bisect_right(prices, q)
            res.append(best[i - 1] if i > 0 else 0)
        return res
