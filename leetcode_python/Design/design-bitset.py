...
# V0
# IDEA : STORE THE BITS *AND* A LAZY "INVERTED" FLAG SO flip() IS O(1)
#
#   physically rewriting every bit on flip() would make the operation O(n),
#   and there can be 10^5 of them. instead keep the raw array plus a boolean
#   `inverted`; the LOGICAL value of bit i is
#       bits[i] ^ inverted
#
#   with `ones` tracking the logical count :
...
#   toString() is the one unavoidable O(n) call.
#
# time = O(1) per op except toString(), which is O(size); space = O(size)
class Bitset(object):
    ...
    def __init__(self, size):
        ...
        self.bits = [0] * size
        self.inverted = 0
        self.ones = 0            # number of LOGICAL 1 bits

    def _get(self, idx):
        return self.bits[idx] ^ self.inverted
