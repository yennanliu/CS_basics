...
#  call -> store them in instance state (self.buf4 / self.i / self.cnt).
#
# time = O(n) per read() call
# space = O(1)
class Solution(object):
    def __init__(self):
        self.buf4 = [""] * 4   # internal scratch buffer
        self.i = 0             # next index to consume inside buf4
        self.cnt = 0           # number of valid chars currently in buf4
    ...
    def read(self, buf, n):
        ...
        """
        j = 0  # how many chars we have written into buf so far
        while j < n:
